...
- 어떤 공휴일이 며칠인지, 어떤 일정이 그 요일에 걸리는지는 **서버가 알아서 계산**합니다.
...
[B] 공휴일 "날짜에는 일정을 하지 않는다"는 명령 — "공휴일/빨간날"에 "~에(는)", "~마다"가 붙어
    ...
    → { "op": "skip_on_holidays", "target_event_ids": ..., "scope": ... } 를 **하나만** 만드세요.
...
async function executeOperation(supabase: any, op: any, ctx: { viewYear: number; viewMonth: number }) {
  ...
  if (op.op === "skip_on_holidays") {
    ...
    const holidayDates = all
      .filter((e: any) => e.is_holiday === true && e.event_date)
